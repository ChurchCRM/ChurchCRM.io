...
faqs:
  ...
  - q: "我需要自己的服务器吗？还是可以将 ChurchCRM 安装在本地计算机上？"
    a: >-
      ...
      运行软件的环境。ChurchCRM 仅支持 LAMP 技术栈（Linux、Apache、MySQL、PHP），必须在
      Linux 环境中运行。不过，如果您没有服务器，有两个实用选择。


      **共享主机（推荐大多数教会使用）：** 在任何主流服务商（Hostinger、Bluehost、SiteGround、
      DreamHost 等）购买共享主机账户，这些服务均运行 Linux，可获得 ChurchCRM 所需的全部
      环境——PHP、MySQL 和 Apache——每月费用约 3–10 美元。大多数共享主机都包含 Softaculous，
      可一键安装 ChurchCRM。这是最常见的部署方式，全体会众可随时从任意设备访问。


      **使用 Docker 的本地计算机（适合测试或单用户使用）：** 在 Windows、Mac 或 Linux 上
      本地运行 ChurchCRM 的最佳方式是使用 Docker——它会自动提供正确的 Linux 和 LAMP 环境，
      无需手动配置服务器。Docker Desktop 免费下载，官方 ChurchCRM Docker 镜像只需一条命令
      即可完成全部配置。重要限制是：本地安装只能在该计算机上访问——除非其他人与您在同一
      局域网内并经过手动配置，否则其他同工无法从自己的设备登录。
